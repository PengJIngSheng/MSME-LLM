...
def _normalise_chart_spec(spec: dict) -> dict:
    ...
    top_colors = [str(c) for c in (spec.get("colors") or []) if str(c).strip()]
    series = []
    for item in raw_series:
        ...
        series.append({
            ...
            "colors": [str(c) for c in (item.get("colors") or []) if str(c).strip()],
        })
    if not labels and series:
        labels = [str(i + 1) for i in range(len(series[0]["values"]))]
    return {
        "type": chart_type if chart_type in {"bar", "pie", "donut", "line", "area", "scatter"} else "bar",
...
def _chart_color(chart: dict, series_idx: int = 0, item_idx: int | None = None) -> str:
    series = chart.get("series") or []
    palette = chart.get("colors") or _CHART_COLORS
    if item_idx is not None and series_idx < len(series):
        item_colors = series[series_idx].get("colors") or []
        if item_idx < len(item_colors) and item_colors[item_idx]:
            return item_colors[item_idx]
    if series_idx < len(series) and series[series_idx].get("color"):
        return series[series_idx]["color"]
    idx = item_idx if item_idx is not None else series_idx
    return palette[idx % len(palette)]

def _axis_lines(parts: list, left: int, top: int, plot_w: int, plot_h: int, min_val: float, max_val: float):
    for tick in range(5):
        frac = tick / 4
        ...
        parts.append(f'<text x="{left - 8}" y="{y + 4:.1f}" font-size="10" text-anchor="end" fill="#64748b">{value:,.0f}</text>')
...
def _render_bar_chart(chart: dict) -> str:
    ...
    min_val = min([0.0] + all_values)
    max_val = max([0.0] + all_values) or 1.0
    if min_val == max_val:
        min_val, max_val = 0.0, max_val or 1.0
    value_span = max_val - min_val
    zero_y = top + plot_h - ((0 - min_val) / value_span) * plot_h
    ...
    _axis_lines(parts, left, top, plot_w, plot_h, min_val, max_val)
    parts.append(f'<line x1="{left}" y1="{zero_y:.1f}" x2="{left + plot_w}" y2="{zero_y:.1f}" stroke="#94a3b8" stroke-width="1.5"/>')
    ...
    for gi, label in enumerate(labels):
        ...
        for si, serie in enumerate(series):
            ...
            value_y = top + plot_h - ((value - min_val) / value_span) * plot_h
            bar_h = abs(zero_y - value_y)
            x = base_x - (len(series) * bar_w) / 2 + si * bar_w
            y = min(value_y, zero_y)
            color = _chart_color(chart, si, gi if len(series) == 1 else None)
    ...
    legend_items = labels if len(series) == 1 else [s["name"] for s in series]
    for si, item_name in enumerate(legend_items[:5]):
        lx = left + si * 150
        ly = height - 10
        color = _chart_color(chart, 0 if len(series) == 1 else si, si if len(series) == 1 else None)
        parts.append(f'<rect x="{lx}" y="{ly - 10}" width="10" height="10" fill="{color}"/>')
        parts.append(f'<text x="{lx + 15}" y="{ly}" font-size="12" fill="#334155">{_svg_text(str(item_name)[:20])}</text>')
    parts.append("</svg>")
    return "".join(parts)

def _render_line_chart(chart: dict, area: bool = False, scatter: bool = False) -> str:
    labels, series = chart["labels"], chart["series"][:4]
    if not labels or not series:
        return ""
    width, height = 760, 360
    ...
    for si, serie in enumerate(series):
        ...
        color = _chart_color(chart, si)
        points = " ".join(f"{x:.1f},{y:.1f}" for x, y in coords)
        if area and coords:
            base_y = top + plot_h
            area_points = f"{coords[0][0]:.1f},{base_y:.1f} {points} {coords[-1][0]:.1f},{base_y:.1f}"
            parts.append(f'<polygon points="{area_points}" fill="{color}" opacity="0.18"/>')
        if not scatter:
...
def _render_donut_chart(chart: dict) -> str:
    svg = _render_pie_chart(chart)
    if not svg:
        return ""
    # The pie renderer uses a fixed center/radius. Overlay a white centre to
    # create a donut while preserving the same legend and slice colors.
    return svg.replace("</svg>", '<circle cx="210" cy="165" r="58" fill="#fff"/><circle cx="210" cy="165" r="58" fill="none" stroke="#e2e8f0" stroke-width="1"/></svg>')
...
def _render_chart_spec(spec: dict) -> str:
    ...
    if chart["type"] == "pie":
        ...
        svg = _render_donut_chart(chart)
    elif chart["type"] == "line":
        svg = _render_line_chart(chart)
    elif chart["type"] == "area":
        svg = _render_line_chart(chart, area=True)
    elif chart["type"] == "scatter":
        svg = _render_line_chart(chart, scatter=True)
    else:
...
async def markdown_to_pdf(markdown_text: str, doc_type: str = "general", is_template: bool = False, filename_stem: str = "") -> tuple:
    """
    ...
    requested_clean = ""
    if filename_stem:
        requested_clean = re.sub(r'[*_`#\[\]]', '', filename_stem)
        requested_clean = re.sub(r'[—–]', '-', requested_clean)
        requested_clean = re.sub(r'[^\w\s\-]', '', requested_clean).strip()
        requested_clean = re.sub(r'\s+', '_', requested_clean)
        requested_clean = requested_clean[:60].rstrip('_') if len(requested_clean) > 60 else requested_clean

    # Build a clean filename from the requested name first, then from the title.
    if requested_clean:
        filename = f"{requested_clean}.pdf"
    elif raw_title:
