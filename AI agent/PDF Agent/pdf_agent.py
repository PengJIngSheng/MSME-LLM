...
_BUSINESS_PLAN_GENERATION_PATTERNS = [
    "create a business plan", "create business plan",
    "generate a business plan", "generate business plan",
    "write a business plan", "write business plan",
    "make a business plan", "make business plan",
    "prepare a business plan", "prepare business plan",
    "draft a business plan", "draft business plan",
    "insert this details", "insert these details",
]
def _is_business_plan_generation_request(t: str) -> bool:
    low = (t or "").lower()
    compact = re.sub(r"\s+", " ", low).strip()
    return (
        "business plan" in compact
        and (
            any(p in compact for p in _BUSINESS_PLAN_GENERATION_PATTERNS)
            or _is_pdf_generation_request(t)
            or _is_direct(t)
        )
    )
...
def _detect_doc_type(text: str) -> str:
    ...
    Returns one of: financial | annual_report | academic | legal |
                    medical   | business_plan | business | general
    """
    t = text.lower()
    ...
    # Business plan requests/templates can also contain financial terms such as
    # cash flow and balance sheet, so detect the explicit plan intent before the
    # broader financial/business buckets.
    if _has("business plan", "startup business plan", "business owner/promoter",
            "business concept/executive summary", "business readiness and sustainability checklist",
            "startup expenses & capitalization", "company description worksheet",
...
def process_agent_request(chat_id: str, user_message: str, attachments: list, history_messages: list = None):
    ...
    if state["stage"] == "init":
        if new_text and current_upload_is_template and state.get("source_data"):
            ...
            state["doc_type"]      = _detect_doc_type(f"{user_message}\n{state['source_data']}")
            _mark_source_document(state, new_attachments)
            if state["doc_type"] == "business_plan" and _is_business_plan_generation_request(user_message):
                state["stage"]           = "generate"
                state["generate_pdf_now"]= True
                state["use_fast_model"]  = False
                state["generation_question_pending"] = False
                state["generation_question_asked"] = True
                state["generation_choice_answered"] = True
                _log("[agent] Business plan requested with source upload → direct generation")
                instruction = _prompts.build_default_generation_instruction(
                    "business_plan",
                    _prompts.get_structure("business_plan"),
                    user_message,
                    reply_lang,
                )
            else:
    ...
    # ══════════════════════════════════════════════════════════════════════════
        ...
        if new_text:
            ...
            state["generation_question_asked"] = True
            state["generation_choice_answered"] = True
            doc_type = _detect_doc_type(f"{user_message}\n{state.get('source_data', '')}")
            if doc_type != state.get("doc_type"):
        ...
        if new_text:
            if _is_template_yes(user_message):
                ...
                if state["doc_type"] == "business_plan" and _is_business_plan_generation_request(user_message):
                    ...
                    state["generation_question_asked"] = True
                    state["generation_choice_answered"] = True
                    _log("[agent] New source PDF during generate + business plan request → direct generation")
                    instruction = _prompts.build_default_generation_instruction(
                        "business_plan",
                        _prompts.get_structure("business_plan"),
                        user_message,
        ...
        if new_text:
            if _is_template_yes(user_message):
                ...
                if state["doc_type"] == "business_plan" and _is_business_plan_generation_request(user_message):
                    ...
                    state["generation_question_asked"] = True
                    state["generation_choice_answered"] = True
                    _log("[agent] New source PDF in done stage + business plan request → direct generation")
                    instruction = _prompts.build_default_generation_instruction(
                        "business_plan",
                        _prompts.get_structure("business_plan"),
                        user_message,
